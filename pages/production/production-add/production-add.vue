<template>
	<view>
		<u--form labelPosition="left" :model="value" :rules="rules" ref="form" labelWidth="105"
			...
			<u-form-item label="询价单编号" prop="askNo" borderBottom v-if="isInquiryToProduction">
				<u--input v-model="value.askNo" border="none" disabled></u--input>
			</u-form-item>
			...
			<!-- <u-form-item v-if="isInquiryToProduction && value.picPath != null" required label="款式老图" prop="customerSku" borderBottom>
				<view style="display: flex; direction: row;">
					<image style="width: 200rpx;height: 200rpx;margin-right: 10px;" mode="scaleToFill" :src="value.picPath"></image>
				<span class="txtClose" style="height: 200rpx; line-height: 200rpx;" @click="deletepicPath">删除</span>
				</view>
			</u-form-item> -->
			...
			<u-line></u-line>
			<!-- <u-form-item v-if="isInquiryToProduction && value.detailPaths.length !=0" required label="详情老图"
				prop="customerSku"></u-form-item>
			<view v-if="isInquiryToProduction">
				<block v-for="(item, index) in value.detailPaths" :key="index" style="display: flex; direction: row;">
					<image style="width: 200rpx;height: 200rpx;margin: 0 15rpx" mode="scaleToFill" :src="item">
					</image>
					<span class="txtClose" style="height: 200rpx; line-height: 200rpx;"
						@click="deletedetailPaths(index)">删除</span>
				</block>
				<u-line></u-line>
			</view> -->
			...
			<u-form-item required label="生产类型" prop="produceTypeId" borderBottom @click="showproduceType = true">
				...
				<text style="font-size: 32rpx;" v-if="isInquiryToProduction">
					{{produceType}}
				</text>
				<text style="font-size: 32rpx;" v-if="!isInquiryToProduction">
					{{produceType}}<!-- {{produceTypeList[produceType-1].value}} -->
				</text>
			</u-form-item>
			...
			<view style="margin-top: 10px;margin-bottom: 10px;"
				...
				<t-table>
					...
					<!-- <t-tr v-for="(item2, index2) in checkboxSizeValue" :key="item2">
						<t-td>{{item2}}</t-td>
						<t-td v-for="(item3, index3) in checkboxColorValue" :key="item3">
							<u-input border="none" inputAlign="center"
								:value="colorAndSizeValue.data[index2][index3].count" type="number"
								@input="changeNum($event, index2, index3)"></u-input>
						</t-td>
					</t-tr> -->
			...
			<!-- <u-form-item required label="交期">
				<view style="width: 200rpx;">
					<u-button type="primary" size="small" @click="addHopeDate()">增加</u-button>
				</view>
			</u-form-item>
			<view style="margin-bottom: 10px;">
				<u-datetime-picker ref="datetimePicker" :show="showDate" :value="value.hopeDates[dateIndex]" mode="date"
					:minDate="minDate" :formatter="formatter" @cancel="showDate = false;dateIndex = -1"
					@confirm="confirmHandler($event, dateIndex)"></u-datetime-picker>
				<t-table>
					<t-tr>
						<t-th>序号</t-th>
						<t-th>日期</t-th>
					...
					<t-tr v-for="(item, index) in value.hopeDates" :key="item">
						<t-td>{{index + 1}}</t-td>
						<t-td @click.native="showDateEvent(index)">{{item | date('yyyy.mm.dd')}}</t-td>
						<t-td>
							<u-button :custom-style="{width: '32px', height: '32px', padding: '0'}" shape="circle"
								:disabled="index === 0" type="primary" icon="trash" @click="delHopeDate(index)">
							</u-button>
						</t-td>
					</t-tr>
				</t-table>
	...
	export default {
		...
		onLoad(options) {
			...
			findListByDictTypeCode('produce_type').then(res => {
				...
				if (options.isInquiryToProduction) {
					for (let i = 0; i < this.produceTypeList.length; i++) {
						if (this.produceTypeList[i].code == options.toproduceType) {
			...
			if (options.isInquiryToProduction) {
				this.isInquiryToProduction = true
				uni.setNavigationBarTitle({
					title: '询价单转生产单发布'
				})
				this.value.askId = options.toddbid
				this.value.askNo = options.toaskId
				this.value.customerSku = options.tocustomerSku
				this.value.categorySmallId = options.tocategorySmallId
				...
				// 		this.produceType = this.produceTypeList[i].value
				// 		this.value.produceTypeId = this.produceTypeList[i].code
				// 		console.log(options.toproduceType,'options.toproduceType');
				// 		console.log(i,'i');
				// 		console.log(this.produceTypeList[i].code,'this.produceTypeList[i].code');
				// 		console.log(this.value.produceTypeId,'this.value.produceTypeId');
				// 	}
				// }
				

				this.toid = options.toddbid

				this.downfile(options.tostylePicStr).then(file => {
					this.value.fileList1.push(file)
				})
				// this.value.fileList1.push(options.tostylePicStr)

				tbimgpicInfosAsk({
					id: this.toid,
				}).then(res => {
					res.map(img => {
						// this.value.detailPaths.push(img)
						this.downfile(img).then(file => {
							this.value.fileList2.push(file)
						})
					})
				})
				/* console.log(this.value.picPath, "value.picPath")
				console.log(this.value.detailPaths, "value.detailPaths") */

				findItemByIdUsingGET({
					id: this.value.categorySmallId
				}).then(res => {
					// console.log(res.typeName)
					let category3 = res.typeName
					this.meid = res.pid
					findItemByIdUsingGET({
						id: this.meid
					}).then(res2 => {
						// console.log(res.typeName)
						let category2 = res2.typeName
						this.biid = res2.pid
						findItemByIdUsingGET({
							id: this.biid
						}).then(res3 => {
							// console.log(res.typeName)
							let category1 = res3.typeName
							this.category = category1 + '-' + category2 + '-' + category3
				...
				findListByPidUsingGET({
					...
					findListByPidUsingGET({
						...
						findListByPidUsingGET({
							...
							// getSizeListById(res3[0].categorySize).then(res4 => {
							// 	for (let i = 0; i < res4.length; i++) {
							// 		this.sizeList.push({
							// 			id: i,
							// 			value: res4[i]
		...
		methods: {
			...
			deletepicPath() {
				this.value.picPath = null
				console.log(this.value.picPath, "value.picPath")
			},
			deletedetailPaths(index) {
				this.value.detailPaths.pop()
				console.log(this.value.detailPaths, "value.detailPaths")
			},
			deletePic(event) {
			...
			submit() {
				...
				if (this.value.fileList1.length == 0 && !this.isInquiryToProduction) {
					uni.$u.toast('请上传款式图')
					return
				}
				if (this.value.fileList1.length == 0 && this.isInquiryToProduction && this.value.picPath == null) {
					uni.$u.toast('请上传款式图')
					return
				}
				// if (this.value.fileList2.length == 0 && !this.isInquiryToProduction) {
				// 	uni.$u.toast('请上传详情图')
				// 	return
				// }
				// if (this.value.fileList2.length == 0 && this.isInquiryToProduction && this.value.detailPaths.length == 0) {
				// 	uni.$u.toast('请上传详情图')
				// 	return
				// }
				...
				this.$refs.form.validate().then(res => {
					...
					let data = {
						askId: this.value.askId ? this.value.askId : null,
					...
					// if (this.isInquiryToProduction) {
					// data.picPath = this.value.picPath || '',
					// data.detailPaths = this.value.detailPaths || []
